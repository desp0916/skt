...
public class TruckEventProcessingTopology extends BaseTruckEventTopology {
	private static final Logger LOG = Logger
			.getLogger(TruckEventProcessingTopology.class);
	...
	public TruckEventProcessingTopology(String configFileLocation)
			throws Exception {
	...
	public void configureKafkaSpout(TopologyBuilder builder) {
		...
		LOG.error("GARY:configureKafkaSpout");
	}

	public void configureHDFSBolt(TopologyBuilder builder) {
		// Use pipe as record boundary
		LOG.error("GARY:configureHDFSBolt");
		...
		builder.setBolt(HDFS_BOLT_ID, hdfsBolt, 2).shuffleGrouping(
				KAFKA_SPOUT_ID);
	}

	public void configureHBaseBolt(TopologyBuilder builder) {
		LOG.error("GARY:configureHBaseBolt");
		TruckHBaseBolt hbaseBolt = new TruckHBaseBolt(topologyConfig);
		builder.setBolt(HBASE_BOLT_ID, hbaseBolt, 2).shuffleGrouping(
				KAFKA_SPOUT_ID);
	}

	public void configureLogTruckEventBolt(TopologyBuilder builder) {
		LOG.error("GARY:configureLogTruckEventBolt");
		LogTruckEventsBolt logBolt = new LogTruckEventsBolt();
		builder.setBolt(LOG_TRUCK_BOLT_ID, logBolt).globalGrouping(
				KAFKA_SPOUT_ID);
	}
	...
	private void buildAndSubmit() {
		...
		try {
//			StormSubmitter.submitTopology("skt", conf, builder.createTopology());
			StormSubmitter.submitTopologyWithProgressBar("skt", conf, builder.createTopology());
	...
	public static void main(String[] str) throws Exception {
		...
		TruckEventProcessingTopology truckTopology = new TruckEventProcessingTopology(
				configFileLocation);
