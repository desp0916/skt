...
public class TruckHBaseBolt implements IRichBolt {
	...
	public void prepare(Map stormConf, TopologyContext context,
		...
		try {
			...
			this.eventsCountTable = connection
					.getTable(EVENTS_COUNT_TABLE_NAME);
	...
	public void execute(Tuple tuple) {
		...
		Timestamp eventTime = (Timestamp) tuple
				.getValueByField(TruckScheme.FIELD_EVENT_TIME);
		...
		try {
			...
			Put put = constructRow(EVENTS_TABLE_NAME, driverId, truckId,
					eventTime, eventType, latitude, longitude);
		...
		if (!eventType.equalsIgnoreCase("normal")) {
			try {
				...
				// getInfractionCountForDriver(driverId);

				incidentTotalCount = this.eventsCountTable
						.incrementColumnValue(Bytes.toBytes(driverId),
								CF_EVENTS_COUNT_TABLE, COL_COUNT_VALUE, 1L);
		...
		collector.emit(tuple, new Values(driverId, truckId, eventTime,
				eventType, longitude, latitude, incidentTotalCount));
	...
	private Put constructRow(String columnFamily, String driverId,
			String truckId, Timestamp eventTime, String eventType,
	...
	private long getInfractionCountForDriver(String driverId) {
		...
		try {
			...
			long count = 0;
			if (result != null) {
				byte[] countBytes = result.getValue(CF_EVENTS_COUNT_TABLE,
						COL_COUNT_VALUE);
				if (countBytes != null) {
